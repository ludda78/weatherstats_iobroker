...
// *** User-Einstellungen **********************************************************************************************************************************
    const WET_DP='wetterstation';          // Name des Pfades in der INfluxDB                        
	const WET_DP_BROKER='0_userdata.0.WetterstationFroggit'; 	// wo liegen die Datenpunkte mit den Daten der Wetterstation im Broker  [default: 0_userdata.0.Wetterstation]                          													 
    ...
    let REKORDWERTE_AUSGABEFORMAT="[WERT] am [TAG] [MONAT] [JAHR]";   /* Wie soll die Ausgabe der Rekordwerte formatiert werden (Template-Vorlage)?
...
// ### Funktionen ###############################################################################################
function main() {
    let originalTimestamp = Date.now(); // Aktuelles Datum speichern
    // Jahresregenmenge unabhängig vom Tageslauf setzen
    if (existsState(WET_DP_BROKER + '.Regen_Jahr')) {
        const regenJahr = getState(WET_DP_BROKER + '.Regen_Jahr').val;
        if (!isNaN(regenJahr)) {
 ...
 if (zeitstempel.getDate() == 1) { 
    ...
    // Monatsspeicherung weiter unten, nachdem die Werte des letzten Tages des Monats nicht mit berücksichitgt wurden
...
sendTo('influxdb.'+INFLUXDB_INSTANZ, 'query', 
    ...
    'from(bucket: "'+INFLUXDB_BUCKET+'") |> range(start: '+(start/1000)+', stop: '+(end/1000)+') |> filter(fn: (r) => r._measurement == "' + WET_DP + '") |> filter(fn: (r) => r._field == "Aussentemperatur"); from(bucket: "'+INFLUXDB_BUCKET+'") |> range(start: '+(start/1000)+', stop: '+(end/1000)+') |> filter(fn: (r) => r._measurement == "' + WET_DP + '") |> filter(fn: (r) => r._field == "Wind_max"); from(bucket: "'+INFLUXDB_BUCKET+'") |> range(start: '+((start+72000)/1000)+', stop: '+(end/1000)+') |> filter(fn: (r) => r._measurement == "' + WET_DP + '") |> filter(fn: (r) => r._field == "Regen_Tag")'
    , function (result) {
	...
	//console.log('Tiefstwert Neu bzw Vortag: ' + Tiefstwert);				
    ...
    //neue Berechnung des Monatsdurchnitts unter Berücksichitgnung der Zeiträume (wie lange eine temp. sich nicht geändert hat)
    let sumWeightedTemps = 0;
    let totalDuration = 0;

    for (let i = 0; i < result.result[0].length - 1; i++) {
        let currentTemp = result.result[0][i]._value;
        let nextTimestamp = result.result[0][i + 1].ts;
        let currentTimestamp = result.result[0][i].ts;

        let duration = (nextTimestamp - currentTimestamp) / 1000; // Dauer in Sekunden
        sumWeightedTemps += currentTemp * duration;
        totalDuration += duration;
    }

    // Letztes Intervall bis Tagesende
    let lastTemp = result.result[0][result.result[0].length - 1]._value;
    let lastTimestamp = result.result[0][result.result[0].length - 1].ts;
    let endOfDayTimestamp = end;

    let lastDuration = (endOfDayTimestamp - lastTimestamp) / 1000; // Dauer in Sekunden
    sumWeightedTemps += lastTemp * lastDuration;
    totalDuration += lastDuration;

    let Temp_Durchschnitt = Number((sumWeightedTemps / totalDuration).toFixed(2));
    console.log('TagesTemp_Durchschnitt (zeitgewichtet): ' + Temp_Durchschnitt);


    //console.log('TagesTemp_Durschnitt: ' + Temp_Durchschnitt);
	//console.log('aktueller_Monat.Temperatur_Durchschnitt: ' + getState(PRE_DP+'.aktueller_Monat.Temperatur_Durchschnitt').val);
	//console.log('aktueller Tag zeitstempel.getDate(): ' + zeitstempel.getDate())
	
    let MonatsTemp_Durchschnitt;

	//Monatserster
	if (zeitstempel.getDate() == 1) { 
		//console.log("Ursprüngliches Datum:" + zeitstempel);
		zeitstempel.setDate(0); // setze Datum auf letzten Tag des VorMonats
		//console.log("Geändertes Datum:"+ zeitstempel);
		MonatsTemp_Durchschnitt = Math.round(((((getState(PRE_DP+'.aktueller_Monat.Temperatur_Durchschnitt').val)*(zeitstempel.getDate()-1))+Temp_Durchschnitt)/zeitstempel.getDate())*100)/100;
		//console.log('MonatsTemp_Durschnitt am Monatsletzen: ' + MonatsTemp_Durchschnitt);
		//console.log('Tag für die Berechnung des TemperaturDurchschnitts_Monat: ' + zeitstempel.setDate(0));	
		let originalTimestamp = Date.now(); // Aktuelles Datum speichern
		zeitstempel.setTime(originalTimestamp); //zurücksetzen des Datum auf aktuelles datum
		//console.log("Zurückgesetztes Datum:"+ zeitstempel);
	}
	if (zeitstempel.getDate() == 2) {
		MonatsTemp_Durchschnitt = Temp_Durchschnitt;
        //console.log('MonatsTemp_Durschnitt am ersten Tag: '  + MonatsTemp_Durchschnitt) 
	}
	if (zeitstempel.getDate() > 2) {
		MonatsTemp_Durchschnitt = Math.round(((((getState(PRE_DP+'.aktueller_Monat.Temperatur_Durchschnitt').val)*(zeitstempel.getDate()-2))+Temp_Durchschnitt)/(zeitstempel.getDate()-1))*100)/100;
        //console.log('MonatsTemp_Durschnitt: '  + MonatsTemp_Durchschnitt) 
    }
...
// Tag des Jahres berechnen
    ...
    //Jahresstatistik
       ...
       // Trockenperiode
        // Aktuelles Datum erhalten
         ...
         console.log('[Trockenperiode] Raw-Wert von Info.Letzter_Regen: ' + letzterRegenStr);
   ...
   function BerechnungTrockenperiode() {
       console.log('[Trockenperiode] letzterRegenDatum: ' + letzterRegenDatum);
    ...
    if (letzterRegenDatum !== null) {
        ...
        console.log('[Trockenperiode] Berechnete Trockenperiode: ' + Trockenperiode_akt);
            ...
            if (Trockenperiode_akt >= Trockenperiode_alt) {
                console.log('[Trockenperiode] Neuer Wert wird gesetzt!');
...
function speichern_Monat() {
    ...
    let initialTemp=getState(WET_DP_BROKER+'.Aussentemperatur').val;
    let jsonSummary = [];
    console.log('Nun werden die Monatswerte gespeichert "speichern_Monat()"...');
    //Datenpunkte lesen
    Tiefstwert=getState(PRE_DP+'.aktueller_Monat.Tiefstwert').val;
    //console.log('Tiefstwert Monat :', + Tiefstwert);
...
function Backup_Jahresstatistik() {
  ...
  "Windböe": Windboee_max,
  "Trockenperiode": Trockenperiode,
  "kalte Tage": kalte_Tage_Jahr,
  "warme Tage": warme_Tage_Jahr,
  "Sommertage": Sommertage_Jahr,
  "heiße Tage": heisse_Tage_Jahr,
  "Frosttage": Frosttage_Jahr,
  "Eistage": Eistage_Jahr,
